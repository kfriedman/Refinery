...
class LocationTranslatorTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $providerRawDataNPC = new ProviderRawData($this->rawDataNPC, '', array('host' => 'localhost'), false, new NDOFilter());
        $providerRawDataPC = new ProviderRawData($this->rawDataPC, '', array('host' => 'localhost'), false, new NDOFilter());

        $rawDataArrayNPC = $providerRawDataNPC->getRawDataArray();
        $dataArrayNPC = $rawDataArrayNPC;
        $rawDataArrayPC = $providerRawDataNPC->getRawDataArray();
        $dataArrayPC = $rawDataArrayPC;

        self::assertArrayHasKey('_enhanced', $dataArrayNPC);
        self::assertSame('58th Street Library', $dataArrayPC['_enhanced']['name_space']);

        $ndoNPC = $this->location->translate($providerRawDataNPC);
        $ndoPC = $this->location->translate($providerRawDataPC);

        self::assertInstanceOf('\\NYPL\\Refinery\\NDO', $ndoNPC);
        self::assertInstanceOf('\\NYPL\\Refinery\\NDO', $ndoPC);
    }
}
?>