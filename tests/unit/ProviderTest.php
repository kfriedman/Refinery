...
class ProviderTest extends \PHPUnit_Framework_TestCase
{
     ...
     */
    public function testReadFailsOnInvalidRawData()
    {
        $ndo = \Mockery::mock('NYPL\Refinery\NDO\Content\Page\BasicPage');
        $ndo->shouldReceive('getNdoType')->andReturn('BasicPage');

        $ndoFilter = \Mockery::mock('NYPL\Refinery\NDOFilter');
        $ndoFilter->shouldReceive('getFilterID')->andReturn(1);

        $ProviderRawData = \Mockery::mock('NYPL\Refinery\ProviderRawData');
        $ProviderRawData->shouldReceive('getRawData')->andReturn(array('type' => 'page', 'data' => 'invalid'));
        ...
        $provider->setProviderRawData($ProviderRawData);
        $provider->readNDO($ndo, $ndoFilter);
    }

    public function testReadSuccessfulFromRawData()
    {
        $ndo = \Mockery::mock('NYPL\Refinery\NDO\Content\Page\BasicPage');
        $ndo->shouldReceive('getNdoType')->andReturn('BasicPage');
        ...
        $provider = \Mockery::mock('NYPL\Refinery\Provider\RESTAPI\D7RefineryServerCurrent')->makePartial();
        $provider->setHost('dev.www.aws.nypl.org');
        $provider->setBaseURL('refinery/api/v0.1');
        $provider->setProviderRawData($ProviderRawData);

        $provider->shouldReceive('readNDO')->andReturn($ndo, $ndoFilter);

        $provider->readNDO($ndo, $ndoFilter);

        self::assertSame($ProviderRawData, $provider->getProviderRawData());
    }
}
