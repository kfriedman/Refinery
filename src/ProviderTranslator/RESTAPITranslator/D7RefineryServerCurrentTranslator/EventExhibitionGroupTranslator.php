 ...
 */
class EventExhibitionGroupTranslator extends D7RefineryServerTranslator implements RESTAPITranslator\ReadInterface
{
    /**
    ...
    {
        return $provider->clientGet('node/event_exhibition', null, $ndoFilter, $allowEmptyResults);
    }

    /**
     * @param ProviderRawData $providerRawData
     *
     * @return NDO\EventExhibitionGroup
     */
    public function translate(ProviderRawData $providerRawData)
    {
        $rawDataArray = $providerRawData->getRawDataArray();

        $ndo = new NDO\EventExhibitionGroup();

        foreach ($rawDataArray as $rawData) {
            $providerTranslator = new EventExhibitionTranslator();
