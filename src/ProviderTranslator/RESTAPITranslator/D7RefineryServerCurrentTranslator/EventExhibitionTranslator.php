 ...
 */
class EventExhibitionTranslator extends D7RefineryServerTranslator implements
  RESTAPITranslator\ReadInterface
{
     ...
     *
     * @return NDO\Content\Node\EventExhibition
     */
    public function translate(ProviderRawData $providerRawData)
    {
        $rawData = $providerRawData->getRawDataArray();

        $ndo = new NDO\Content\Node\EventExhibition($rawData['nid']);
        ...
        if (isset($rawData['field_project_image']['und'])) {
            $image = new NDO\Content\Image($rawData['field_project_image']['und'][0]['fid']);
            $ndo->setImage($image);
        }
