 ...
 */
class StaffPickItemGroupTranslator extends StaffPicksServerTranslator
    implements RemoteJSONTranslator\ReadInterface
{
    ...
    public function read(RemoteJSON $provider, NDOFilter $ndoFilter, $allowEmptyResults = false)
    {
        ...
        if ($ndoFilter->getFilterID()) {
            $rawData = array_intersect_key($rawData['items'], array_flip($ndoFilter->getFilterID()));
        } else {
     ...
     *
     * @return NDO\StaffPickItemGroup
     */
    public function translate(ProviderRawData $providerRawData)
    {
        $rawDataArray = $providerRawData->getRawDataArray();

        $ndo = new NDO\StaffPickItemGroup();

        foreach ($rawDataArray as $rawData) {
            $providerTranslator = new StaffPickItemTranslator();
